...
CREATE TABLE if not exists `tbl_sys_role` (
      ...
      `role_lock` bit(1) NOT NULL  default 1 comment '0 无锁 1 锁定',
      `status` bit(1) NOT NULL  default 1 comment '0 无效 1有效',
      `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8  comment='系统角色信息';
...
CREATE TABLE if not exists `tbl_sys_user_role` (
      ...
      `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8  comment='系统用户角色关联信息';
...
CREATE TABLE if not exists `tbl_sys_role_menu` (
      ...
      `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
      PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8  comment='系统角色菜单表';
